...
배경이미지 = pg.image.load('img/배경1.png')
배경이미지 = pg.transform.scale(배경이미지, (화면가로길이, 화면세로길이))

시간이미지 = pg.image.load('img/시간-바.png')
시간이미지크기 = (337, 89)
시간이미지 = pg.transform.scale(시간이미지, 시간이미지크기)

코인이미지 = pg.image.load('img/코인-바.png')
코인이미지크기 = (337, 89)
...
광석스프라이트리스트 = []
광석스프라이트리스트.append(genSprite(광석이미지리스트[-1], (200, 200), 광석최대상태))
...
while 실행여부:
    ...
    for 광석_스프라이트 in 광석스프라이트리스트:
        ...
        광석_스프라이트.rect.x += random.randint(1, 10) - 6
        광석_스프라이트.rect.y += random.randint(1, 10) - 6
        if 광석_스프라이트.rect.x > 화면가로길이 - 152:
            광석_스프라이트.rect.x = 화면가로길이 - 152
        if 광석_스프라이트.rect.x < 152:
            광석_스프라이트.rect.x = 152
        if 광석_스프라이트.rect.y > 화면세로길이 - 152:
            광석_스프라이트.rect.y = 화면세로길이 - 152
        if 광석_스프라이트.rect.y < 152:
            광석_스프라이트.rect.y = 152
    ...
    파워글자 = 작은글꼴.render(str(소울곰파워업그레이드횟수), True, 검은색)
    화면.blit(파워글자, (500, 화면세로길이 - 능력치이미지크기[1] + 55))
    속도글자 = 작은글꼴.render(str(소울곰속도업그레이드횟수), True, 검은색)
    ...
    if 광석자동생성남은시간 <= 0:
        생성위치 = [random.random() * (화면가로길이 - 게임요소크기[0]),
                random.random() * (화면세로길이 - 시간이미지크기[1] - 능력치이미지크기[1] - 게임요소크기[1]) + 시간이미지크기[1]]
        광석스프라이트리스트.append(genSprite(광석이미지리스트[-1], 생성위치, 광석최대상태))
        광석자동생성남은시간 = 광석자동생성시간
